...
public class FiveDaysAheadRule implements Rule {
	...
	private boolean isExpired(LocalDate paymentDate, LocalDate today) {
		...
		return isPaymentYearOutOfRange(paymentDate, expirationDate)
				|| isPaymentMonthOutOfRange(paymentDate, expirationDate)
				|| isPaymentDateOutOfRange(paymentDate, expirationDate);
	}
	...
	private boolean isPaymentDateOutOfRange(LocalDate paymentDate, LocalDate expirationDate) {
		return paymentDate.getDayOfYear() - expirationDate.getDayOfYear() > 0;
	}
